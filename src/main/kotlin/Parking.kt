...
data class Parking(val vehicles: MutableSet<Vehicle>, val maxSpaces: Int = 20) {
    ...
    //-------------------------------------
    /* Function that: check-out a vehicle,
                      calculate their fee,
    ...
    private fun calculateFee(vehicleType: VehicleType, parkedTime: Double) : Int {
        return when {
            parkedTime <= 120 -> vehicleType.startPrice
                ...
                val calculateExtra = ceil((((parkedTime - 120) / 15))).toInt() * 5
    ...
    fun totalEarnings() {
        println("${earnings.first} vehicles have been check out and have earnings of $${earnings.second} ")
    }
